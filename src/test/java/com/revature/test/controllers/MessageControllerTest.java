...
public class MessageControllerTest {
	...
	public void testCreateMessage() throws Exception {
		...
		RequestBuilder request = MockMvcRequestBuilders.post("/api/message/createmessage")
				.accept(MediaType.APPLICATION_JSON_VALUE).content(om.writeValueAsString(mockMessageCreation))
				.contentType(MediaType.APPLICATION_JSON);
		MvcResult result = mockmvc.perform(request).andReturn();

		assertThat(result.getResponse().getContentAsString())
				.isEqualTo(om.writeValueAsString(ClientMessageUtil.CREATION_SUCCESSFUL));
	}
	...
	public void testGetById() throws Exception {
		...
		MvcResult result = mockmvc.perform(request).andReturn();
		assertThat(result.getResponse().getContentAsString()).isEqualTo(om.writeValueAsString(mockMessage1));
	}
...
// @Disabled("Disabled until CreateMessageTest is up!")
	public void testUpdateMessage() throws Exception {
		...
		RequestBuilder request = MockMvcRequestBuilders.put("/api/message/update")
				.accept(MediaType.APPLICATION_JSON_VALUE).content(om.writeValueAsString(mockMessageModification))
				.contentType(MediaType.APPLICATION_JSON);
		MvcResult result = mockmvc.perform(request).andReturn();
		assertThat(om.writeValueAsString(ClientMessageUtil.UPDATE_SUCCESSFUL))
	...
	public void testDeleteMessage() throws Exception {
		...
		RequestBuilder request = MockMvcRequestBuilders.delete("/api/message/delete")
				.accept(MediaType.APPLICATION_JSON_VALUE).content(om.writeValueAsString(mockMessageDeletion))
				.contentType(MediaType.APPLICATION_JSON);
		MvcResult result = mockmvc.perform(request).andReturn();
		assertThat(om.writeValueAsString(ClientMessageUtil.DELETION_SUCCESSFUL))
				.isEqualTo(result.getResponse().getContentAsString());
